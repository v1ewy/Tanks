...
#define FLOATS_PER_VERTEX  4
    ...
    for (const char* p = text; *p && vertexCount < MAX_CHARS * VERTICES_PER_CHAR; p++)
    {
        ...
        if (ch >= 32 && ch < 128) {
            ...
            q.x0 = ox + (q.x0 - ox) * scale;
            q.x1 = ox + (q.x1 - ox) * scale;
            q.y0 = oy + (q.y0 - oy) * scale;
            q.y1 = oy + (q.y1 - oy) * scale;
            currentX = ox + (cx2 - ox) * scale;
            currentY = cy2;
...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (gameState == GAME_STATE_NICK_INPUT) {
            ...
            player_update(window, deltaTime, currentTime, fieldX, fieldY, FIELD_SIZE);
            int spawn_count = 0;
            for (int j = 0; j < GRID_SIZE; j++)
                for (int i = 0; i < GRID_SIZE; i++)
                    if (map[j][i] == 6) spawn_count++;
            bots_update(deltaTime, currentTime, fieldX, fieldY, FIELD_SIZE, sp_bots, spawn_count);
